...
xdg-play - play an XDG event identifier

=head1 SYNOPSIS

B< xdg-play> [{B<-p>|B<--play>}]  [I<OPTIONS>] [--] I<EVENTID> [...]
 B<xdg-play> {B<-s>|B<--show>}    [I<OPTIONS>]
 B<xdg-play> {B<-w>|B<--which>}   [I<OPTIONS>] [--] I<EVENTID> [...]
 B<xdg-play> {B<-W>|B<--whereis>} [I<OPTIONS>] [--] I<EVENTID> [...]
 B<xdg-play> {B<-l>|B<--list>}    [I<OPTIONS>]
 B<xdg-play> {B<-h>|B<--help>}    [I<OPTIONS>] [--] I<EVENTID> [...]
 B<xdg-play> {B<-V>|B<--version>}
 B<xdg-play> {B<-C>|B<--copying>}

=head1 DESCRIPTION

B<xdg-play> takes one or more arguments.  For each of its arguments, it
prints to standard output the full path of the sound file that will
be played and then plays the sound file.  It does this by searching for
a sound entry in the appropriate sub-directories of the C<XDG_DATA_DIRS>
and C<XDG_DATA_HOME> environment variables in accordance with the
L<XDG Sound Theme Specification>.
...
The default is to match the I<EVENTID> and play the sound.  When any of
...
When specified, or no command option is specified, for each I<EVENTID>
argument, play the sound file that corresponds to the event identifier
according to the XDG sound theme specification.  This is accomplished by
searching for the event identifier is the appropriate sub-directories of
the C<XDG_DATA_DIRS> and C<XDG_DATA_HOME> environment variables.

=item B<-w>, B<--which>

For each I<EVENTID> argument, prints to standard output the full path of
the sound file that would have been played when this argument was
supplied to B<xdg-play(1)>.  It does this by searching for an event
identifier in the appropriate sub-directories of the C<XDG_DATA_DIRS>
and C<XDG_DATA_HOME> environment variables using the same algorithms as
L<xdg-play(1)>.

See also, L<xdg-sound-which(1)>, that provides the same function.

=item B<-W>, B<--whereis>

For each I<EVENTID> argument, locates the sound files for the specified
event identifiers.  The supplied names are first stripped of leading
path-name components and any (single) trailing extension of the form
F<.ext> (for example: F<.wav>).  B<xdg-play> then attempts to locate the
desired sound file in the standard Linux places, and in the places
specified by C<XDG_DATA_DIRS> and C<XDG_DATA_HOME>.

See also, L<xdg-sound-whereis(1)>, that provides the same function.

=item B<-s>, B<--show>

List all of the event identifiers and sound files associated with the
current sound theme.  When B<--standard> is specified, list only the
standard event identifiers from the L<XDG Sound Naming Specification>.

See also, L<xdg-sound-list(1)>, that provides the same function.
...
Output list of effective lookup paths that B<xdg-play> is using.
...
The following environment variables are examined by B<xdg-play>:

=over

=item B<HOME>

Used by B<xdg-play> to determine the user's home directory.

=item B<XDG_DATA_HOME>

Used by B<xdg-play> to determine the user's XDG data directory.
...
Used by B<xdg-play> to determine the system XDG data directories.
