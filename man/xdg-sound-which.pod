...
xdg-snd-which - show the full path of XDG sound entries

=head1 SYNOPSIS

B< xdg-snd-which> [I<OPTIONS>] [--] I<EVENTID> [...]
 B<xdg-snd_which> [I<OPTIONS>] {B<-l>|B<--list>}
 B<xdg-snd_which> {B<-h>|B<--help>}
 B<xdg-snd_which> {B<-V>|B<--version>}
 B<xdg-snd_which> {B<-C>|B<--copying>}

=head1 DESCRIPTION

B<xdg-snd-which> takes one or more arguments.  For each of its
...
Specify the audio profile to use when looking up event identifiers.
...
B<xdg-sound-which> does not yet search for localized sounds.
