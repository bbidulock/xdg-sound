...
Command default_command = CommandShow;
...
main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long_only(argc, argv, "snlaotOTN:P:SxXMrdc:u:m:i:I:D::v::hVCH?", long_options,
				     &option_index);
#else				/* _GNU_SOURCE */
		c = getopt(argc, argv, "snlaotOTN:P:SxXMrdc:u:m:i:I:DvhVCH?");
#endif				/* _GNU_SOURCE */
		if (c == -1) {
			if (options.debug)
