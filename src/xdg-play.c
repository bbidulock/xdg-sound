...
Command default_command = CommandPlay;
...
main(int argc, char *argv[])
{
	...
	if (!strcmp(buf, "xdg-sound")) {
		default_command = CommandPlay;
	} else if (!strcmp(buf, "xdg-sound-list")) {
		default_command = CommandShow;
	} else if (!strcmp(buf, "xdg-sound-whereis")) {
		default_command = CommandWhereis;
	} else if (!strcmp(buf, "xdg-sound-which")) {
		default_command = CommandWhich;
	}
	...
	while (1) {
		...
		c = getopt_long_only(argc, argv, "pswWnlaotOTN:P:SxXMrdc:u:m:i:I:D::v::hVCH?", long_options,
				     &option_index);
#else				/* _GNU_SOURCE */
		c = getopt(argc, argv, "pswWnlaotOTN:P:SxXMrdc:u:m:i:I:DvhVCH?");
#endif				/* _GNU_SOURCE */
		if (c == -1) {
			if (options.debug)
